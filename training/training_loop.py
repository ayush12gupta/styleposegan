...
def training_loop(
    ...
    if rank == 0:
        ...
        A_in = torch.zeros([batch_gpu, 3, G.img_resolution, G.img_resolution], device=device)
        P_in = torch.zeros([batch_gpu, 3, G.img_resolution, G.img_resolution], device=device)
        img = misc.print_module_summary(G, [A_in, P_in, c])
    ...
    ddp_modules = dict()
    for name, module in [('G_mapping', G.mapping), ('G_synthesis', G.synthesis), ('ANet', G.ANet), ('D', D), (None, G_ema), ('augment_pipe', augment_pipe)]:
    ...
    if rank == 0:
        ...
        grid_size, img_s, img_t, A_s, A_t, P_s, P_t, labels = setup_snapshot_image_grid(training_set=training_set)
        save_image_grid(img_t, os.path.join(run_dir, 'reals.png'), drange=[0,255], grid_size=grid_size)
        grid_c = torch.from_numpy(labels).to(device).split(batch_gpu)
        A_s = torch.from_numpy(A_s).to(device).split(batch_gpu)
        P_t = torch.from_numpy(P_t).to(device).split(batch_gpu)
        images = torch.cat([G_ema(A=a_s.float(), P=p_t.float(), c=c, noise_mode='const').cpu() for a_s, p_t, c in zip(A_s, P_t, grid_c)]).numpy()
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            phase_img_s, phase_img_t, phase_ap_s, phase_ap_t, phase_pose_s, phase_pose_t, phase_real_c = next(training_set_iterator)
        ...
        for phase, phase_gen_c in zip(phases, all_gen_c):
            ...
            for round_idx, (img_s, img_t, ap_s, ap_t, pose_s, pose_t, real_c, gen_c) in enumerate(zip(phase_img_s, phase_img_t, phase_ap_s, phase_ap_t, phase_pose_s, phase_pose_t, phase_real_c, phase_gen_c)):
