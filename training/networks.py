...
# Residual Block for PNet and ANet
...
class PNet(torch.nn.Module):
    def __init__(self, block=ResidualBlock, layers=[2,2,2]):
        ...
        self.layer0 = self.make_layer(block, 16, layers[0], 2)
        self.layer1 = self.make_layer(block, 64, layers[0], 2)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, P, **block_kwargs): #changes to be made to other modules as well
        block_ws = []
...
class Generator(torch.nn.Module):
    ...
    def forward(self, A, P, c, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
        ...
        img = self.synthesis(ws, P, **synthesis_kwargs)
        return img
